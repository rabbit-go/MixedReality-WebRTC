...
public:
	...
	{
		// I typically do a round-up to the next power of two with square textures for a generic
		// pool, but since we're generally working with video textures, I do expect a certain amount
		// of regularity in size, and we want to avoid the aggressive rounding we might see for
		// certain resolutions (e.g. 720p => 2048x2048), so we use the VideoDesc directly.
		...
		if (bufferIdArray.size())
		{
			...
			{
				Log_Warning("Failed to get staging texture. Invalid entry in free-texture array.");
			}
		}
	...
	{
		for (auto itr = std::begin(m_outstandingBuffers); itr != std::end(m_outstandingBuffers); ++itr)
		{
			if ((*itr).BufferId == bufferId)
			{
				uint64_t thisFrameId = m_lastFrameId + 1;
				m_unsafeBuffers.push({ thisFrameId + FramesUntilSafe, bufferId, (*itr).Desc });
	...
	void ProcessEndOfFrame(uint64_t frameId)
	{
		...
		{
			Log_Warning("There should be no outstanding textures at the end of a frame.");
		}

		// Promote previously used buffers from unsafe to safe, if enough time has passed.
		while (m_unsafeBuffers.size() && m_unsafeBuffers.front().SafeOnFrameId <= frameId)
		{
			...
			if (itr == std::end(m_freeBuffers))
			{
				Log_Warning("Invalid texture found in delay queue. Refusing to place it back in the free list.");
			}
			else
			{
		...
		// Remove textures that are very old. We use the free array like a stack, so the bottom
		// textures are the ones way look at for removal.
		for (auto& freeTexturePair : m_freeBuffers)
		{
			std::deque<FreeStruct>& freeTextureArray = freeTexturePair.second;
			while (freeTextureArray.size() && freeTextureArray.front().LastUsedFrameId + FramesUntilDelete <= frameId)
			{
...
private:
	// The number of frames we wait before we assume a used staging buffer is free again. Three is
	// larger than typical frame queues.
	static const uint64_t FramesUntilSafe = 3;

	// The number of frames we wait before we delete textures. This is pretty lazy. We could improve
	// this by using configurable size limits.
	...
	// Textures that have been used as staging textures but not enough frames have passed for us to consider them safe for re-use.
	std::queue<UnsafeStruct> m_unsafeBuffers;

	// Textures that have been requested as staging textures and have not yet been reclaimed.
...
public:
    ...
    virtual void EndModifyTexture(void* dstTexture, const TextureUpdate& update, const VideoDesc& desc, const std::vector<VideoRect>& rects = {});
...
std::shared_ptr<RenderApi> CreateRenderApi_OpenGLCoreES(UnityGfxRenderer apiType)
{
...
bool RenderApi_OpenGLCoreES::BeginModifyTexture(const VideoDesc& desc, TextureUpdate* update)
{
...
#else
	GLsizeiptr size = desc.Height * desc.Width * GetBytesPerPixel(desc.Format);
	GLubyte* ptr = (GLubyte*)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, size, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
#endif
...
void RenderApi_OpenGLCoreES::EndModifyTexture(void* dstTexture, const TextureUpdate& update, const VideoDesc& desc, const std::vector<VideoRect>& rects)
{
