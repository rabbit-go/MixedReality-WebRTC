...
bool RenderApi_D3D12::BeginModifyTexture(const VideoDesc& desc, TextureUpdate* update)
{
    ...
    // Fill data
    const UINT64 kDataSize = desc.Width * desc.Height * GetBytesPerPixel(desc.Format);
...
void RenderApi_D3D12::EndModifyTexture(void* dstTexture, const TextureUpdate& update, const VideoDesc& desc, const std::vector<VideoRect>& rects)
{
    ID3D12Device* device = s_D3D12->GetDevice();
    ...
    {
        D3D12_BOX srcBox{ rect.X, rect.Y, 0, rect.X + rect.Width, rect.Y + rect.Height, 0 };
        s_D3D12CmdList->CopyTextureRegion(&dstLoc, rect.X, rect.Y, 0, &srcLoc, &srcBox);
    }
